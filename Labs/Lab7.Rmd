...
Visualizing the time series with scatter and Line plot:
...
Visualizing the time series with line plot:
...
Visualizing the time series with line and area plot:
...
Visualizing multiple time series with scatter plot.
...
```{r}
ggplot(preprint_arc3, aes(x = as.Date(date), y = count, color = archive)) +
  geom_point() +
  theme_classic() +
  labs(x = "year",
       y = "preprints/month",
...
Visualizing multiple time series with scatter and line plot:
...
Visualizing multiple time series with line plot:
...
# Visualizing multiple time series in a facet setup

Facet plots allow us to draw multiple groups on separate plots instead of drawing them on a single plot, arranged vertically, horizontally, or in a grid.
...
```{r}
ggplot(preprint_arc3, aes(x = as.Date(date), y = count)) +
  geom_line() +
...
To arrange facet plots side-by-side, specify the relevant variable in the col 
argument within the facet_grid() function:
...
Visualizing trend smoothing adjusting the smoothness:
